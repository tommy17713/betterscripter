...
function Script(_record){
			...
			function loadRequireFromUrl(_url, _scriptId, callback) {
				$.ajax({
					...
					error:function(request, status, error) {
						alert("Chrome User Script Handler Warning - Unable to load @require:\n\n " + _url);
						callback();
					}
				});					
			}
	...
	function _parseExcludes(){
		...
		if (headers.exclude instanceof Array) 
			...
			if(!!headers.exclude) excludes.push(headers.exclude);
		}
	...
	function _parseIncludes(){
		...
		if (headers.include instanceof Array) 
			...
			if(!!headers.include) includes.push(headers.include);
		}
		if (headers.match instanceof Array) 
			for (var i = 0; i < headers.match.length; i++) 
				includes.push(headers.match[i]);
		else {
			if(!!headers.match) includes.push(headers.match);
		}
	...
	function _parseRequires(){
		...
		if (headers.require instanceof Array) 
			for (var i = 0; i < headers.require.length; i++) 
				requires.push(headers.require[i].trim());
		else 
			if (headers.require) 
				requires.push(headers.require.trim());
	...
	function _parseSiteIcons(){
		...
		for (var i = 0; i < domains.length; i++) {
			...
			if(domain.indexOf('*') < -1) { html += '<a href="http://' + faviconDomain + '" target="_blank">'; }
	...
	this.getRequireSourceByUrl = function(url) {
		url = url.trim();
		if(url.substring(0, 2) !== "ht") {
			url = "https:" + url;
		}
		var requireSource = localStorage.getItem('@require_' + this.id + '_' + slugify(url));
		if(!requireSource)
			requireSource = localStorage.getItem('@require ' + slugify(url));	// old require storage method
	...
	this.setRequireSourceByUrl = function(url, source) {
		...
		localStorage.removeItem('@require_' + this.id + '_' + slugify(url));
		localStorage.setItem('@require_' + this.id + '_' + slugify(url), source);
		localStorage.removeItem('@require ' + slugify(url));
		localStorage.setItem('@require ' + slugify(url), source);
	}
	...
	this.hasAllRequires = function() {
		...
		for(var i in requireUrls) {
			hasAll = localStorage.getItem('@require_' + this.id + '_' + slugify(requireUrls[i].trim())) ? hasAll : false;
		}
		return hasAll;
	}
