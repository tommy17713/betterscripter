/* Chrome User Script Handler [https://github.com/stefanXO/Script-Handler]
 * Based on Jerome Dane's Blank Canvas Script Handler <http://blankcanvas.me/contact/>  
 * 
 * This file is part of the Chrome User Script Handler. See readme.md for
...
function toggleSourceCollapsed(id, e) {
	e.preventDefault();
	e.stopPropagation();
	var editBox = $("#"+id);
...
function updateRequireList() {
	...
	for(var i = 0; i < requireUrls.length; i++) {
		var requireURL = requireUrls[i].trim();
		var requireSlug = slugify(requireURL);
		var source = script.getRequireSourceByUrl(requireURL);
		if (source != null && $('#editRequiresWrapper p[name="toggler_' + requireSlug + '"]').size() == 0) {
			var clickEl = $('<p name="toggler_' + requireSlug + '" title="Edit ' + requireURL + '"><img src="' + chrome.extension.getURL("images/bullet_expand.png") + '"/> ' + requireURL + '</p>');
			$('#editRequiresWrapper').append(clickEl);
			clickEl.on("click", toggleSourceCollapsed.bind(this, 'require_' + requireSlug));
			$('#editRequiresWrapper').append('<textarea wrap="off" spellcheck="false" data-gramm="false" name="' + requireSlug + '" id="require_' + requireSlug + '" style="display:none;">' + source.replace(/</g, '&lt;').replace(/>/g, '&gt;') + '</textarea>');
			//$('#editRequiresWrapper textarea[name="' + requireSlug + '"]').keydown(textareaKeyHandler);
		}
	}
...
function saveCurrentScript(callback) {
	...
	for(var i = 0; i < requireUrls.length; i++) {
		...
		if(textarea.size() == 1) {
			currentScript.setRequireSourceByUrl(requireUrls[i], textarea.val());
		}
	}	
